/**
 * Purchase routes. Legacy form id 10, form code 301.
 *
 * Ports PurchaseController: Index / Create / Save / Edit / Print.
 */
...
const PERM = formPermissions(10, 301);
...
export default async function purchaseRoutes(app: FastifyInstance): Promise<void> {
  app.get('/', {
    ...
    handler: async (req) => {
      ...
      let base = db.selectFrom('purchase').leftJoin('supplier', 'supplier.id', 'purchase.sup_id');

      if (!req.principal.isSuperAdmin) {
        base = base.where('purchase.branch_id', '=', req.principal.branchId);
      } else if (q.branchId !== undefined) {
        base = base.where('purchase.branch_id', '=', q.branchId);
      }

      if (dates.from) base = base.where('purchase.date', '>=', dates.from);
      if (dates.to) base = base.where('purchase.date', '<=', dates.to);
  ...
  app.get('/form-data', {
    ...
    handler: async (req) => {
      ...
      let suppliers = db
        .selectFrom('supplier')
        .select(['id', 'name', 'phone', 'account_no'])
  ...
  app.get('/:id', {
    ...
    handler: async (req) => {
      ...
      if (!req.principal.isSuperAdmin && purchase.branch_id !== req.principal.branchId) {
  ...
  app.post('/preview', {
    preHandler: app.requireAction(PERM.formId, PERM.view),
    handler: async (req) => service.computeTotals(purchaseBody.parse(req.body)),
  });

  app.post('/', {
    preHandler: app.requireAction(PERM.formId, PERM.create),
    handler: async (req, reply) => {
      const result = await service.createPurchase(req.principal, purchaseBody.parse(req.body));
  ...
  app.put('/:id', {
    ...
    handler: async (req) => {
      const { id } = idParam.parse(req.params);
      return service.updatePurchase(req.principal, id, purchaseBody.parse(req.body));
    },
  });
  ...
  app.get('/:id/print', {
    ...
    handler: async (req) => {
      const { id } = idParam.parse(req.params);
      return loadPrintDocument(req.principal, 'purchase', id);
    },
  });
}
